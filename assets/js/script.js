...
(function($){
	...
	$.ajax({
		...
		success: function (d) {
			...
			$("#title_hero").html(data["title"]);

			$.each(data["team"], function(index, el) {
				$(".team-name").html(el["name"]);
				$(".team-title").html(el["bio"]);
				$("#facebook").attr("href",el["facebook_link"]);
				$("#linkedin").attr("href",el["linkedin_link"]);
				$("#twitter").attr("href",el["twitter_link"]);
